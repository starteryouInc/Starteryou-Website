...
const storeMetadataInDB = async () => {
  try {
    ...
    for (const [modelName, model] of Object.entries(models)) {
      ...
      const sampleData = await collection.findOne({}, { projection: { _id: 0 } });
      ...
      const MetadataModel = mongoose.model(`${modelName}Metadata`, new mongoose.Schema({}, { strict: false }), `${modelName.toLowerCase()}_metadata`);
      ...
      console.log(`✅ Metadata stored in '${modelName.toLowerCase()}_metadata' collection`);
    }
