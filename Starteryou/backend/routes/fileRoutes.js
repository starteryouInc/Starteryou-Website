...
const mongoURI = process.env.MONGODB_URI; // Ensure MongoDB URI is correctly set
...
router.post('/upload', upload.single('file'), async (req, res) => {
  ...
  if (!uploadedBy) {
    return res.status(400).json({ message: 'The "uploadedBy" field is required' });
  }
  ...
  uploadStream.on('finish', async (file) => {
    try {
      ...
      res.status(201).json({ file: { _id: file._id, filename: originalname, title } });
    } catch (error) {
...
router.put('/update', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    uploadStream.on('finish', async (file) => {
      console.log('New version of file uploaded. Old ID:', oldFileId, 'New ID:', file._id);
      ...
      res.status(200).json({ file: { _id: file._id, filename: originalname, title } });
    });
...
router.get('/', async (req, res) => {
  try {
    const files = await FileMetadata.find().select('title originalFilename uploadedBy createdAt');
