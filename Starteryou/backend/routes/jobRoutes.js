...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const cacheKey = `/api/v1/jobportal/jobs/fetch-job?${JSON.stringify(req.query)}`;
...
//       // Construct cache key based on job ID
//       const cacheKey = `/api/fetch-job/${id}`;
//       console.log(`Cache Key: ${cacheKey}`);
...
//           if (!job) {
//             throw new Error("No job found by particular ID"); // Prevents caching empty results
//           }
//           return job;
//         },
//         cacheConfig.defaultTTL
...
//       res.status(500).json({
//         success: false,
//         msg: "Some error occurred while fetching this Job",
//         error: error.message,
//       });
//     }
//   }
